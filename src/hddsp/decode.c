                      ...
                      uint8_t *puncture, int puncture_len)
{
    ...
    for (i = 0, j = 0; i < frame_len; i++, j += 3)
    {
        ...
        if (puncture[j % puncture_len] && ((coded[j] > 0) != PARITY(r & g1)))
            errors++;
        if (puncture[(j+1) % puncture_len] && ((coded[j+1] > 0) != PARITY(r & g2)))
            errors++;
        if (puncture[(j+2) % puncture_len] && ((coded[j+2] > 0) != PARITY(r & g3)))
            errors++;
    }
