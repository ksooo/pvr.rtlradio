...
void DecoderAdapter::PADChangeDynamicLabel(const CDynamicLabel& dl)
{
  ...
  if (!dl.m_dynamicLabel.empty())
    id3TagData.emplace_back("TKDL", dl.m_dynamicLabel);
  ...
  if (!dl.m_playItems.empty())
  {
    ...
    {
      const std::string id3v2ID = CDynamicLabel::DLPlusContentTypes[item.first].id3v2;

      if (id3v2ID.empty())
        continue;

      if (!item.second.empty())
        id3TagData.emplace_back(id3v2ID, item.second);
      else
        id3TagData.emplace_back(id3v2ID, "!!!EMPTY!!!");
  ...
  {
    id3TagData.emplace_back("TIT2", "!!!EMPTY!!!");
  }
...
void DecoderAdapter::PADChangeSlide(const std::shared_ptr<MOT_FILE>& slide)
{
    mot_file_t mot_file;

    mot_file.data = slide->data;
    mot_file.content_main_type = slide->content_main_type;
    mot_file.content_full_type = slide->content_full_type;
