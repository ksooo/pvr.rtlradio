...
SuperframeFilter::SuperframeFilter(SubchannelSinkObserver* observer, bool decode_audio, bool enable_float32) : SubchannelSink(observer, "aac") {
	...
	//pFile = fopen("C:/Dev-Dbg/test.bin", "w");
}
...
void SuperframeFilter::Feed(const uint8_t *data, size_t len) {
	...
	for(int i = 0; i < num_aus; i++) {
		...
		//fwrite(au_data, sizeof(uint8_t), au_len, pFile);
...
// --- RSDecoder -----------------------------------------------------------------
RSDecoder::RSDecoder() {
	rs_handle = init_rs_dab(8, 0x11D, 0, 1, 10, 135);
	if(!rs_handle)
		throw std::runtime_error("RSDecoder: error while init_rs_char");
}

RSDecoder::~RSDecoder() {
	free_rs_dab(rs_handle);
}
...
//	sf[20] ^= 0xFF;
	...
	for(int i = 0; i < subch_index; i++) {
		...
		// detect errors
		int corr_count = decode_rs_dab(rs_handle, rs_packet, corr_pos, 0);
		if(corr_count == -1)
			uncorr_errors = true;
		else
