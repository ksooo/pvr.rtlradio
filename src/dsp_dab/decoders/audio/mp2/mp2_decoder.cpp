...
const int MP2Decoder::table_nbal_48a[] = {4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 3, 3, 3,
                                          3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2};
// from ETSI TS 103 466, table 5 (= ISO/IEC 11172-3, table B.2c):
const int MP2Decoder::table_nbal_48b[] = {4, 4, 3, 3, 3, 3, 3, 3};
// from ETSI TS 103 466, table 6 (= ISO/IEC 13818-3, table B.1):
const int MP2Decoder::table_nbal_24[] = {4, 4, 4, 4, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2,
                                         2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
const int* MP2Decoder::tables_nbal[] = {table_nbal_48a, table_nbal_48b, table_nbal_24};
...
MP2Decoder::MP2Decoder(SubchannelSinkObserver* observer, bool float32)
  : SubchannelSink(observer, "mp2")
{
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_init: " +
                             std::string(mpg123_plain_strerror(mpg_result)));
  ...
  if (!handle)
    throw std::runtime_error("MP2Decoder: error while mpg123_new: " +
                             std::string(mpg123_plain_strerror(mpg_result)));
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_format_none: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  mpg_result = mpg123_format(handle, 48000, MPG123_MONO | MPG123_STEREO,
                             float32 ? MPG123_ENC_FLOAT_32 : MPG123_ENC_SIGNED_16);
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_format #1: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  mpg_result = mpg123_format(handle, 24000, MPG123_MONO | MPG123_STEREO,
                             float32 ? MPG123_ENC_FLOAT_32 : MPG123_ENC_SIGNED_16);
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_format #2: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  // disable resync limit
  mpg_result = mpg123_param(handle, MPG123_RESYNC_LIMIT, -1, 0);
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_param: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  mpg_result = mpg123_open_feed(handle);
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_open_feed: " +
                             std::string(mpg123_plain_strerror(mpg_result)));
}
...
void MP2Decoder::Feed(const uint8_t* data, size_t len)
{
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_feed: " +
                             std::string(mpg123_plain_strerror(mpg_result)));
  ...
  do
  {
    ...
    switch (mpg_result)
    {
      ...
      default:
        throw std::runtime_error("MP2Decoder: error while mpg123_framebyframe_next: " +
                                 std::string(mpg123_plain_strerror(mpg_result)));
    }
...
size_t MP2Decoder::DecodeFrame(uint8_t** data)
{
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_framedata: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  // forwarding the whole frame (except ScF-CRC + F-PAD) as X-PAD, as we don't know the X-PAD len here
  observer->ProcessPAD(body_data, body_bytes - FPAD_LEN - scf_crc_len, false,
                       body_data + body_bytes - FPAD_LEN);
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_framebyframe_decode: " +
                             std::string(mpg123_plain_strerror(mpg_result)));

  return frame_len;
}

void MP2Decoder::ProcessUntouchedStream(const unsigned long& header,
...
bool MP2Decoder::CheckCRC(const unsigned long& header,
                          const uint8_t* body_data,
                          const size_t& body_bytes)
{
  ...
  if (mpg_result != MPG123_OK)
    throw std::runtime_error("MP2Decoder: error while mpg123_info: " +
                             std::string(mpg123_plain_strerror(mpg_result)));
...
void MP2Decoder::ProcessFormat()
{
  ...
  scf_crc_len =
      (info.version == MPG123_1_0 && info.bitrate < (info.mode == MPG123_M_MONO ? 56 : 112)) ? 2
                                                                                             : 4;
