...
template<class T>
using aligned_ptr = std::unique_ptr<T, aligned_deleter_t>;

template<class T, class... Args>
inline aligned_ptr<T> make_aligned(Args&&... args)
{
#ifdef _MSC_VER
	auto ptr = _aligned_malloc(sizeof(T), alignof(T));
	if(ptr == nullptr) throw std::bad_alloc();
#else
	void* ptr = nullptr;
	if(posix_memalign(&ptr, alignof(T), sizeof(T)) != 0) throw std::bad_alloc();
#endif

	try {

		auto instance = ::new(ptr) T(std::forward<Args>(args)...);
		return aligned_ptr<T>(instance);
	}
	catch(...) {
